...
TEST(HashJoinTest, UseFuture) {
  ...
  auto register_task_group_callback = [&](Task task, TaskCont cont) {
    int task_group_id = task_groups.size();
    auto task_wrapped = [task = std::move(task), &thread_ids](size_t thread_id, int64_t task_id) {
      thread_ids.insert(std::this_thread::get_id());
      return task(thread_id, task_id);
    };
    task_groups.emplace(task_group_id, std::make_pair(std::move(task), std::move(cont)));
  ...
  auto start_task_group_callback = [&](int task_group_id, int64_t num_tasks) {
    ...
    for (int64_t task_id = 0; task_id < num_tasks; task_id++) {
      ...
      task_futures.push_back(folly::via(&e).thenValue([dop, task, task_id](folly::Unit) { return task(task_id, task_id); }));
    }
    folly::collectAll(task_futures).via(&e).thenValue([task_group_id, &task_groups](auto&& results) {
