...
class PipelineLogger : public PipelineObserver {
 public:
  Status OnPipelineTaskBegin(const PipelineTask& pipeline_task, size_t plex,
                             const PipelineContext&, const task::TaskContext&,
                             ThreadId thread_id) override {
    ARA_LOG(INFO) << "Pipeline task " << pipeline_task.Name() << " plex " << plex
  ...
  Status OnPipelineSourceBegin(const PipelineTask& pipeline_task, size_t plex,
                               const PipelineContext&, const task::TaskContext&,
                               ThreadId thread_id) override {
  ...
  Status OnPipelinePipeBegin(const PipelineTask& pipeline_task, size_t plex, size_t pipe,
                             const PipelineContext&, const task::TaskContext&,
                             ThreadId thread_id,
  ...
  Status OnPipelineDrainBegin(const PipelineTask& pipeline_task, size_t plex, size_t pipe,
                              const PipelineContext&, const task::TaskContext&,
                              ThreadId thread_id) override {
    auto pipe_name = pipeline_task.Pipeline().Plexes()[plex].pipe_ops[pipe]->Name();
    ARA_LOG(INFO) << "Pipeline task " << pipeline_task.Name() << " plex " << plex
  ...
  Status OnPipelineSinkBegin(const PipelineTask& pipeline_task, size_t plex,
                             const PipelineContext&, const task::TaskContext&,
                             ThreadId thread_id,
                             const std::optional<Batch>& input) override {
